...
class Guitester ( name: String, scope: CoroutineScope  ) : ActorBasicFsm( name, scope ){
		...
		return { //this:ActionBasciFsm
				state("s0") { //this:State
					action { //it:State
						...
						request("dostorefood", "dostorefood(25)" ,"serviceaccessgui" )  
				...
				state("sendticket") { //this:State
					action { //it:State
						CommUtils.outmagenta("$name | sono arrrivato in sendticket")
						if( checkMsgContent( Term.createTerm("ticketaccepted(TICKETCODE,TICKETSECRET,TIMESTAMP)"), Term.createTerm("ticketaccepted(TICKETCODE,TICKETSECRET,TIMESTAMP)"), 
								...
								request("doinsertticket", "doinsertticket($Ticketcode,$Ticketsecret)" ,"serviceaccessgui" )  
						}
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
